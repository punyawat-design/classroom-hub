...
export default function StudentCourseDetailPage(){
  ...
  useEffect(()=>{(async()=>{
    const [{data:c,error:ce},{data:m,error:me},{data:a,error:ae}] = await Promise.all([
      ...
      supabase.from("learning_materials")
        .select("id,title,description,link_url,file_name,storage_path,course_id,courses(name)")
        .eq("course_id",courseId)
        .order("created_at",{ascending:false}),
      supabase.from("announcements")
        .select("id,title,body,created_at,course_id,classroom_id,courses(name),classrooms(name)")
        .eq("course_id",courseId)
        .order("created_at",{ascending:false})
        .limit(30)
    ]);
  ...
  const announcementText = useMemo(()=>{
    return announcements.length===0 ? "ยังไม่มีประกาศในวิชานี้" : "";
  },[announcements]);
  ...
  return <>
    ...
    <header className="page-header">
      <div>
        ...
        <p>สื่อการสอนและประกาศของวิชานี้เท่านั้น</p>
      </div>
    </header>
    ...
    <section className="section">
      ...
      <div className="cards-list">
        {materials.map(m=><article className="card material-card" key={m.id}>
          <div>
            <h3>{m.title}</h3>
            <div className="muted small">{m.courses?.name}</div>
            <p>{m.description}</p>
          </div>
          <button className="btn primary" onClick={()=>setSelected(m)}>ดูสื่อ / ดาวน์โหลด</button>
        </article>)}
        {materials.length===0&&<div className="empty card">วิชานี้ยังไม่มีสื่อการสอน</div>}
      </div>
    </section>
    ...
    <section className="section">
      <h2>ประกาศล่าสุด</h2>
      <div className="cards-list">
        {announcements.map(a=><article className="card" key={a.id}>
          <h3>{a.title}</h3>
          <div className="muted small">
            {a.classrooms?.name||"ทุกห้อง"} • {thaiDate(a.created_at)}
          </div>
          <p>{a.body}</p>
        </article>)}
        {announcementText&&<div className="empty card">{announcementText}</div>}
      </div>
    </section>
